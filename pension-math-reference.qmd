...
project_salary <- function(entry_salary, entry_age, current_age, 
                          merit_scale_current, merit_scale_entry,
                          inflation_rate, productivity_rate) {
...
calculate_pbo_end <- function(pbo_begin, service_cost, interest_cost, 
                             actuarial_gains_losses, plan_amendments, 
                             benefits_paid) {
  pbo_end <- pbo_begin + service_cost + interest_cost + 
             actuarial_gains_losses + plan_amendments - benefits_paid
  return(pbo_end)
}
```
...
calculate_final_average_benefit <- function(years_of_service, final_salaries, 
                                           benefit_multiplier) {
...
calculate_required_contribution <- function(normal_cost, ual, 
                                           amortization_period, discount_rate) {
  if (ual <= 0) {
    return(normal_cost)
  }
  ...
  if (discount_rate == 0) {
    ...
    amortization_factor <- discount_rate / (1 - (1 + discount_rate)^(-amortization_period))
...
example_contribution <- calculate_required_contribution(50000, example_ual, 15, 0.07)
...
PensionPlan <- R6Class("PensionPlan",
  public = list(
    participants = NULL,
    assets = NULL,
    assumptions = NULL,
    ...
    calculate_actuarial_liability = function(discount_rate = 0.07) {
      if (nrow(self$participants) == 0) return(0)
      ...
      for (i in 1:nrow(self$participants)) {
        ...
        projected_benefit <- participant$salary * 1.02^(65 - participant$age) * 
                            participant$service * 0.015
        
        # Present value (simplified - assumes retirement at 65)
        years_to_retirement <- max(0, 65 - participant$age)
        pv_benefit <- projected_benefit / (1 + discount_rate)^years_to_retirement
    ...
    annual_valuation = function() {
      ...
      funded_ratio <- if (actuarial_liability > 0) self$assets / actuarial_liability else Inf
