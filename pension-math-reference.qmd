...
```{r}
#| label: plan-level-validation
# Validate fundamental equation for entire plan
validate_plan_level <- function(
  participants_df,
  mortality_table,
  service_table,
  discount_rate = 0.07,
  ...
  for (i in 1:nrow(participants_df)) {
    ...
    individual_validation <- validate_single_participant(
      entry_age = p$entry_age,
      current_age = p$current_age,
      retirement_age = 65, # Assume common retirement age
      entry_salary = p$entry_salary,
      benefit_rate = benefit_rate,
      discount_rate = discount_rate,
      salary_growth_rate = 0.04, # Assume 4% salary growth
      mortality_table = mortality_table,
      service_table = service_table
    )

    # Accumulate totals
    total_pvfb <- total_pvfb + individual_validation$pvfb
    total_al <- total_al + individual_validation$al
    total_nc <- total_nc + individual_validation$nc
    total_pvfnc <- total_pvfnc + individual_validation$pvfnc
  }

  # Plan-level validation
  equation_rhs <- total_al + total_nc + total_pvfnc
  difference <- abs(total_pvfb - equation_rhs)
  relative_error <- difference / total_pvfb

  results <- list(
    total_participants = nrow(participants_df),
    total_pvfb = total_pvfb,
    total_al = total_al,
    total_nc = total_nc,
    total_pvfnc = total_pvfnc,
    equation_lhs = total_pvfb,
    equation_rhs = equation_rhs,
    difference = difference,
    relative_error = relative_error,
    validates = difference < 0.01 * total_pvfb
  )
...
sample_participants <- data.frame(
  id = 1:5,
  entry_age = c(25, 30, 22, 28, 35),
  current_age = c(35, 45, 32, 38, 50),
  entry_salary = c(50000, 60000, 45000, 55000, 70000)
)

# Run plan-level validation
plan_validation <- validate_plan_level(
  sample_participants,
  mortality_table,
  service_table
)
...
cat("\n=== PLAN-LEVEL VALIDATION ===\n")
cat("Total Participants:", plan_validation$total_participants, "\n\n")
cat("PVFB (Left side):", round(plan_validation$total_pvfb, 2), "\n")
cat(
  "AL + NC + PVFNC (Right side):",
  round(plan_validation$equation_rhs, 2),
  "\n"
)
cat("  - Total AL:", round(plan_validation$total_al, 2), "\n")
cat("  - Total NC:", round(plan_validation$total_nc, 2), "\n")
cat("  - Total PVFNC:", round(plan_validation$total_pvfnc, 2), "\n\n")
cat("Difference:", round(plan_validation$difference, 2), "\n")
cat("Relative Error:", round(plan_validation$relative_error * 100, 3), "%\n")
cat(
  "Validation Result:",
  ifelse(plan_validation$validates, "PASS", "FAIL"),
  "\n"
)
```

## Cost Method Consistency Validation

```{r}
#| label: cost-method-validation
# Validate that different cost methods produce consistent total costs
validate_cost_methods <- function(participant, mortality_table, service_table) {
  # Unit Credit Method
  uc_results <- validate_single_participant(
    entry_age = participant$entry_age,
    current_age = participant$current_age,
    retirement_age = 65,
    entry_salary = participant$entry_salary,
    benefit_rate = 0.015,
    discount_rate = 0.07,
    salary_growth_rate = 0.04,
    mortality_table = mortality_table,
  ...
  # Entry Age Normal Method (simplified)
  # Spread the PVFB evenly over the career
  career_length <- 65 - participant$entry_age
  years_elapsed <- participant$current_age - participant$entry_age
  years_remaining <- 65 - participant$current_age

  ean_normal_cost <- uc_results$pvfb / career_length
  ean_al <- ean_normal_cost * years_elapsed
  ean_pvfnc <- ean_normal_cost * years_remaining

  # Validate EAN fundamental equation
  ean_equation_rhs <- ean_al + ean_normal_cost + ean_pvfnc
  ean_difference <- abs(uc_results$pvfb - ean_equation_rhs)

  cat("=== COST METHOD COMPARISON ===\n")
  cat("Unit Credit Method:\n")
  cat("  AL:", round(uc_results$al, 2), "  NC:", round(uc_results$nc, 2), "\n")
  cat("Entry Age Normal Method:\n")
  cat("  AL:", round(ean_al, 2), "  NC:", round(ean_normal_cost, 2), "\n\n")

  cat("Both methods produce same PVFB:", round(uc_results$pvfb, 2), "\n")
  cat("EAN Equation validates:", abs(ean_difference) < 0.01, "\n")

  return(list(
    uc = uc_results,
    ean = list(al = ean_al, nc = ean_normal_cost, pvfnc = ean_pvfnc)
  ))
}

# Test with first participant
cost_method_validation <- validate_cost_methods(
  sample_participants[1, ],
  mortality_table,
  service_table
)
```

## Asset-Liability Reconciliation Validation

```{r}
#| label: asset-liability-reconciliation
# Demonstrate asset and liability movements over time
demonstrate_reconciliation <- function() {
  # Starting position
  beginning_assets <- 1000000
  beginning_al <- 900000

  # Annual movements
  normal_cost <- 75000
  interest_on_al <- beginning_al * 0.07 # 7% interest on liability
  contributions <- 80000
  benefit_payments <- 40000
  asset_return_rate <- 0.08 # 8% return on assets

  # Asset reconciliation
  expected_asset_earnings <- beginning_assets * 0.07 # Expected 7% return
  actual_asset_earnings <- beginning_assets * asset_return_rate
  asset_gain <- actual_asset_earnings - expected_asset_earnings

  ending_assets <- beginning_assets +
    contributions +
    actual_asset_earnings -
    benefit_payments

  # Liability reconciliation
  ending_al <- beginning_al + normal_cost + interest_on_al - benefit_payments

  # Funded status changes
  beginning_funded_status <- beginning_assets - beginning_al
  ending_funded_status <- ending_assets - ending_al
  change_in_funded_status <- ending_funded_status - beginning_funded_status

  cat("=== ASSET-LIABILITY RECONCILIATION ===\n\n")

  cat("ASSETS:\n")
  cat("Beginning Assets:", format(beginning_assets, big.mark = ","), "\n")
  cat("+ Contributions:", format(contributions, big.mark = ","), "\n")
  cat(
    "+ Investment Return:",
    format(actual_asset_earnings, big.mark = ","),
    "\n"
  )
  cat("- Benefit Payments:", format(benefit_payments, big.mark = ","), "\n")
  cat("= Ending Assets:", format(ending_assets, big.mark = ","), "\n\n")

  cat("LIABILITIES:\n")
  cat("Beginning AL:", format(beginning_al, big.mark = ","), "\n")
  cat("+ Normal Cost:", format(normal_cost, big.mark = ","), "\n")
  cat("+ Interest Cost:", format(interest_on_al, big.mark = ","), "\n")
  cat("- Benefit Payments:", format(benefit_payments, big.mark = ","), "\n")
  cat("= Ending AL:", format(ending_al, big.mark = ","), "\n\n")

  cat("FUNDED STATUS:\n")
  cat("Beginning:", format(beginning_funded_status, big.mark = ","), "\n")
  cat("Ending:", format(ending_funded_status, big.mark = ","), "\n")
  cat("Change:", format(change_in_funded_status, big.mark = ","), "\n\n")

  cat("ANALYSIS:\n")
  cat("Asset Gain/(Loss):", format(asset_gain, big.mark = ","), "\n")
  cat(
    "Contribution vs NC:",
    format(contributions - normal_cost, big.mark = ","),
    "\n"
  )
  ...
  return(list(
    ...
    ending_al = ending_al,
    funded_status_change = change_in_funded_status,
    asset_gain = asset_gain
  ))
}

reconciliation_example <- demonstrate_reconciliation()
```
